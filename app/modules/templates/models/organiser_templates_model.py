...
class OrganiserTemplate(Base):
    ...
    organizer_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), index=True, nullable=False)
    platform_template_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("platform_templates.id", ondelete="CASCADE"), index=True, nullable=False)
    ...
    share_slug: Mapped[str | None] = mapped_column(String(255), unique=True, nullable=True)
    ...
    organiser: Mapped["User"] = relationship("User", back_populates="organiser_templates")
